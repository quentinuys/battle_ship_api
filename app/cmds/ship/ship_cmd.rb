...
private
	...
	def update_ship(name, status, x, y, direction)
		empty_ship = @game.ships.where{(name == my{name}) & (status == "live")}.first
		exist_ship = @game.ships.where{(name == my{name}) & (status == "live") & (location_x == my{x}) & (location_y == my{y}) }.first
	...
	def create_ships_random
		ship_types.each do |ship|
			...
			@game.ships.create!(name: ship[:name], size: ship[:size], status: "live", location_x: rand_ship[:x], location_y: rand_ship[:y], direction: rand_ship[:direction])
		end
	end
